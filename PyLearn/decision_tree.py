...
class DT():
    def __init__(self, train_input=None, train_output=None, testdata=None, N=3):
    ...
    def train(self, train_input=None, train_output=None):
        ...
        self._root=self.divide(self._train_input, self._train_output)
    ...
    def divide(self, train_input, train_output):
        ...
        for dim in self._dims:
            ...
            if select_entropy==0:
                break
        if len(self._dims)==1:
            lefty=[y for x,y in zip(train_input,train_output) if x[select_dim]<=select_value]
            righty=[y for x,y in zip(train_input,train_output) if x[select_dim]>select_value]
            leftcls=collections.Counter(lefty).most_common(1)[0][0]
            if len(righty)==0:
                rightcls=leftcls
            else:
                rightcls=collections.Counter(righty).most_common(1)[0][0]
            node=Node(select_dim,select_value,leftcls=leftcls,rightcls=rightcls)
        else:
            leftx=np.array([x for x,y in zip(train_input,train_output) if x[select_dim]<=select_value])
            lefty=[y for x,y in zip(train_input,train_output) if x[select_dim]<=select_value]
            rightx=np.array([x for x,y in zip(train_input,train_output) if x[select_dim]>select_value])
            righty=[y for x,y in zip(train_input,train_output) if x[select_dim]>select_value]
            self._dims.remove(select_dim)
            if leftEntropy==0:
                leftcls=collections.Counter(lefty).most_common(1)[0][0]
                leftnode=None
            else:
            ...
            if rightEntropy==0:
                ...
                rightcls=None
            node=Node(select_dim,select_value,leftnode,rightnode,leftcls,rightcls)
            self._dims.append(select_dim)
        return node
    def evaluate(self, x):
        node=self._root
        while 1:
